...
strings_cs_path = r"C:\Antigravity_proyectos\Lector e IDE\LectorGlobalApp\Strings.cs"
xaml_path = r"C:\Antigravity_proyectos\Lector e IDE\LectorGlobalApp\MainWindow.xaml"
...
new_es = {
    ...
    "LblAtajo1Desc": "Mantenlo presionado para decir algo corto (o leer la selección)",
...
with open(strings_cs_path, "r", encoding="utf-8") as f:
    content = f.read()

# Append to dictionaries
for k, v in new_es.items():
    if k not in content:
        content = content.replace('{ "TabInsightsTitle", "Estadísticas de Uso" }', f'{{ "TabInsightsTitle", "Estadísticas de Uso" }},\n                {{ "{k}", "{v}" }}')

for k, v in new_en.items():
    if k not in content:
        content = content.replace('{ "TabInsightsTitle", "Usage Statistics" }', f'{{ "TabInsightsTitle", "Usage Statistics" }},\n                {{ "{k}", "{v}" }}')

# Add properties
props = ""
for k in new_es.keys():
    if f"public string {k} =>" not in content:
        props += f"        public string {k} => Get(\"{k}\");\n"

content = content.replace("public event PropertyChangedEventHandler PropertyChanged;", props + "\n        public event PropertyChangedEventHandler PropertyChanged;")

with open(strings_cs_path, "w", encoding="utf-8") as f:
    f.write(content)
...
replacements = {
    ...
    'Content="Enviar Feedback"': 'Content="{Binding BtnSendFeedbackAction}"'
}
...
print("Textos internacionalizados exitosamente.")
